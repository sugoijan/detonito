...
pub struct StatefulSolver {
    cfg: SolverConfig,
    /// Key: sorted `SmallVec<[EquationId; 8]>` (component identity).
    component_cache: HashMap<SmallVec<[EquationId; 8]>, CachedComponent>,
...
impl StatefulSolver {
    ...
    pub fn solve(
        ...
        {
            let mut hidden_var_ids: Vec<CellVarId> =
                build.problem.variables.iter().map(|v| v.id).collect();
            hidden_var_ids.sort_unstable();
            self.certain_mines
                .retain(|id| hidden_var_ids.binary_search(id).is_ok());
            self.certain_safe
                .retain(|id| hidden_var_ids.binary_search(id).is_ok());
        }
        ...
        // BTreeSet iteration is already sorted, so collect directly to a Vec.
        let initial_problem = if !self.certain_mines.is_empty() || !self.certain_safe.is_empty() {
            let certain_mines_vec: Vec<CellVarId> = self.certain_mines.iter().copied().collect();
            let certain_safe_vec: Vec<CellVarId> = self.certain_safe.iter().copied().collect();
            build
                .problem
                .apply_prior_assignments(&certain_mines_vec, &certain_safe_vec)
        } else {
            build.problem
        };
...

        {
            ...
            // Phase 2: 2-query SAT per connected component (with solver cache)
            {
                ...
                for component in &current_problem.components {
                    ...
                    // Sort in-place so we can use binary_search for membership tests.
                    unresolved_vars.sort_unstable();
                        ...
                        .get_mut(component.equation_ids.as_slice())
                    {
                        ...
                        let mut phase1_ids: Vec<CellVarId> =
                            phase1_priors.iter().map(|(id, _)| *id).collect();
                        phase1_ids.sort_unstable();

                        for (&var_id, &is_mine) in &self.cumulative_assignments {
                            if cached.handle.all_var_ids.binary_search(&var_id).is_ok()
                                && unresolved_vars.binary_search(&var_id).is_err()
                                && phase1_ids.binary_search(&var_id).is_err()
                            {
        ...
        if !all_mines_ids.is_empty() || !all_safe_ids.is_empty() {
            let mut mines_sorted = all_mines_ids.clone();
            ...
            if !current_problem.variables.is_empty() {
                ...
                if !cascade.contradiction {
                    ...
                    if !cascade.forced_safe.is_empty() || !cascade.forced_mines.is_empty() {
                        let mut cascade_mines = cascade.forced_mines.clone();
                        cascade_mines.sort_unstable();
                        let mut cascade_safe = cascade.forced_safe.clone();
                        cascade_safe.sort_unstable();
                        current_problem =
    ...
    fn prune_stale_cache(&mut self, problem: &ConstraintProblem) {
        self.component_cache.retain(|key, _| {
            problem
                .components
                .iter()
                .any(|c| c.equation_ids.as_slice() == key.as_slice())
        });
    }
}
...
/// are dropped. Sorts copies of the slices and delegates to [`reduce_equations_inplace`].
fn reduce_equations(
    ...
    let mut safe_sorted = safe_ids.to_vec();
    safe_sorted.sort_unstable();
    let mut mine_sorted = mine_ids.to_vec();
    mine_sorted.sort_unstable();
    reduce_equations_inplace(equations, &safe_sorted, &mine_sorted);
}
