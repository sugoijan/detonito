...
pub struct ConstraintComponent {
    pub variable_ids: Vec<CellVarId>,
    pub equation_ids: Vec<EquationId>,
}
...
impl ConstraintProblem {
    ...
    pub(crate) fn apply_prior_assignments(
        mut self,
        prior_mines: &BTreeSet<CellVarId>,
        prior_safe: &BTreeSet<CellVarId>,
    ) -> Self {
        ...
        self.variables
            .retain(|v| !prior_mines.contains(&v.id) && !prior_safe.contains(&v.id));
...
pub(crate) fn reduce_equations_inplace(
    ...
    safe: &BTreeSet<CellVarId>,
    mines: &BTreeSet<CellVarId>,
) {
    ...
    equations.retain_mut(|eq| {
        let mines_removed = eq
            .variable_ids
            .iter()
            .filter(|id| mines.contains(id))
            .count() as u16;
        eq.variable_ids
            .retain(|id| !mines.contains(id) && !safe.contains(id));
...
pub(crate) fn build_components(
    ...
    let mut touched_vars = BTreeSet::new();
    ...
    let touched_list: Vec<_> = touched_vars.iter().copied().collect();
    ...
    for var in variables {
        if !touched_vars.contains(&var.id) {
...
mod tests {
    ...
    fn apply_prior_assignments_decrements_target_only_for_mines() {
        ...
        let prior_mines: BTreeSet<CellVarId> = [a].into_iter().collect();
        let prior_safe: BTreeSet<CellVarId> = BTreeSet::new();
        ...
        assert_eq!(
            reduced.equations[0].variable_ids,
            vec![c],
        ...
        let prior_mines2: BTreeSet<CellVarId> = BTreeSet::new();
        let prior_safe2: BTreeSet<CellVarId> = [c].into_iter().collect();
        ...
        assert_eq!(reduced2.equations[0].variable_ids, vec![a]);
    }
}
